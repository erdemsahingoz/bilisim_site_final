...
export default function ProductCard({ product }) {
  ...
  // Resim kaynagı belirleniyor:
  // imagePath backend'den geliyorsa → o kullanılır
  // yoksa → placeholder gösterilir
  const imageUrl = product.imagePath && product.imagePath.trim() !== ""
    ? product.imagePath
    : `/images/placeholder.png`;

  const handleImageError = (e) => {
    e.target.onerror = null;
    e.target.src = `https://via.placeholder.com/600x400/0f1724/00e5ff?text=${encodeURIComponent(product.name || "Urün")}`;
  };
